...
export default function Header({ onOpenLogin, onOpenRegister, onOpenCart, isVisible }: HeaderProps) {
  ...
  return (
    <>
      <header
        className={`bg-white shadow-lg sticky top-0 z-50 transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'
          }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <nav className="hidden lg:flex items-center space-x-6">
              <a href="/" className={`block text-gray-700 hover:text-purple-600 font-medium transition-colors px-3 py-2 rounded-lg hover:bg-purple-50 ${location.pathname === '/' ? 'text-purple-600 bg-purple-100' : ''}`}>Inicio</a>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 lg:hidden" onClick={() => setIsMobileMenuOpen(false)}>
          <div className="fixed top-0 right-0 h-full w-4/5 max-w-sm bg-white shadow-lg p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <nav className="flex flex-col space-y-4">
              <a href="/" className={`block text-gray-700 hover:text-purple-600 font-medium transition-colors px-3 py-2 rounded-lg hover:bg-purple-50 ${location.pathname === '/' ? 'text-purple-600 bg-purple-100' : ''}`} onClick={() => setIsMobileMenuOpen(false)}>Inicio</a>
